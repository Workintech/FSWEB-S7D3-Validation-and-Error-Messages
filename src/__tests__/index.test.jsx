...
test("Login component'inde reactstrap'ten FormFeedback kullanılmış", () => {
  expect(loginPage.includes('<FormFeedback')).toBe(true);
});

test("Login component'inde errors ve isValid state'leri tanımlanmış", () => {
  expect(loginPage.includes('[errors,setErrors]')).toBe(true);
  expect(loginPage.includes('[isValid,setIsValid]')).toBe(true);
});

test("Login component'inde isValid başlangıç değeri false yapılmış", () => {
  expect(loginPage.includes('const[isValid,setIsValid]=useState(false)')).toBe(
    true
  );
});

test("Login component'inde useEffect kullanılmış tanımlanmış", () => {
